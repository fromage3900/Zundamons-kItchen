...
local function craftItem(player, item, position, quality)
	if not RemoteRateLimiter.allow(player, "craft", CRAFT_COOLDOWN) then
		return "Fail"
	end
	if typeof(item) ~= "string" or typeof(position) ~= "Vector3" then
		return "Fail"
	end
	if quality ~= nil and typeof(quality) ~= "string" then
		return "Fail"
	end
	quality = quality or "ok"
	if not QUALITY_BONUS[quality] then
		quality = "ok"
	end
