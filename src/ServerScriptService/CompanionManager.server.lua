...
local function buildCompanion(player, compType)
	...
	pl.Color = def.glow
	Tween:Create(pl, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), { Brightness = 2.2 }):Play()
...
setCompEv.OnServerEvent:Connect(function(player, compType)
	if not CompanionConfig.companions[compType] then
		return
	end
	local data = PlayerDataService.getOrCreate(player)
	local def = CompanionConfig.getCompanion(compType)
